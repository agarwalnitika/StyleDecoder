...
function TextInput() {
  ...
  const [heading, setHeading] = useState("Output");
  ...
  const handleKeyPress = (event) => {
    ...
    if (event.ctrlKey) {
      ...
      if (event.keyCode === 49) {
        model = "old";
        setHeading("Output: old");
        axios
          .post(
            "http://34.172.89.50:8000/get_suggestions",
            {
              text: event.target.value.trim().length
                ? event.target.value
                : "<BOS>",
              model: model,
              max_length: "15",
    ...
    <div className="flex h-screen w-screen items-center justify-center">
      <div className="flex gap-[5px]">
        <div className="inline-flex flex-col mr-[85px]">
          ...
          <textarea
            placeholder="Press Ctrl + 0 for new model. Press Ctrl + 1 for old model"
            className="border border-solid border-blue w-[500px] h-[250px] outline-none bg-blue-light text-grey p-[15px] text-xl font-medium rounded-[4px]"
            style={{ resize: "none" }}
            name="text"
            onKeyDown={(e) => {
              handleKeyPress(e);
      ...
      <div className="cursor-pointer bg-blue text-white w-[147px] h-[36px] items-center justify-center inline-flex text-xl rounded-[4px]" onClick={() => {
        ...
        <div className="inline-flex flex-col justify-start max-w-[350px] w-[350px]">
          <h1 className="text-4xl font-bold mb-[10px]">{heading}</h1>
          <div className="mb-[40px]">
            <div className="text-xl text-blue font-medium">Suggestion 1</div>
            <div className="text-xl text-grey  font-medium">{tone1}</div>
          </div>
          <div className="mb-[40px]">
            <div className="text-xl text-blue font-medium">Suggestion 2</div>
            <div className="text-xl text-grey  font-medium">{tone2}</div>
          </div>
          <div className="mb-[40px]">
            <div className="text-xl text-blue font-medium">Suggestion 3</div>
